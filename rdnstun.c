...
static void rdnstun (
  ...
  for (int pollres; !*shutdown;) {
    ...
    unsigned char ipver = ((struct iphdr *) pkt_receive)->version;
    int ret;
    switch (ipver) {
        case 4:
...
undefined_ipver:
      logger(RDNSTUN_NAME, LOG_LEVEL_DEBUG,
             "Received IPv%d packet but no v%d chains defined", ipver, ipver);
    }
    if (0) {
unknown_ipver:
...
fail_reply:
      switch (ret) {
        case 11:
          ...
          break;
        case 12:
          logger(RDNSTUN_NAME, LOG_LEVEL_DEBUG, "Received packet with TTL 0");
          break;
        case 13:
